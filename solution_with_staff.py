...
def analyse(capture,nodes_queue,nodes_staff,out_video,seconds_in_queue):
    ...
    while True:
        ...
        def get_nodes_from_user(object_name,orig_image):
            ...
            cv2.putText(image,object_name,(50, 50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,255),2,cv2.LINE_AA)
            nodes=[]
        ...
        for i in range(n):
            if result.boxes.cls[i]==0:
                p=[int(result.boxes.xywh[i][0]),int(result.boxes.xywh[i][1]+result.boxes.xywh[i][3]/2)]
                # Draw results on this image: person rect and bottom point
                cv2.rectangle(image,(int(result.boxes.xyxy[i][0]),int(result.boxes.xyxy[i][1])),(int(result.boxes.xyxy[i][2]),int(result.boxes.xyxy[i][3])),(0,255,0),2)
        ...
        print("Queue:",counter_in,". Delayed:",counter_in_delayed,". Staff:",counter_staff,". Others:",counter_out,".")
        ...
        if counter_warning!=0:
            print(f"Warning: there are {counter_warning} persons in both polygons! Your camera looks misconfigured!")
            warned=True
        cv2.putText(image,str(counter_in)+":"+str(counter_in_delayed)+(" !" if warned else ""),(50, 50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,255),2,cv2.LINE_AA)
...
if nodes[0] is None or nodes[1] is None:
    print("No correct detection was performed. Create correct camera's settings file using this script.")
    exit(1)
# Opportunity to move resulting file
out_path=input(f"Where do you whant to place analysed video (path or NONE; current is {output_default_path}): ").strip()
if out_path!="NONE":
